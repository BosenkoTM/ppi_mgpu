...
Конечно! Вот описание двух технологических стеков, четко разделенных по задачам: первый — для подготовки данных и ручной разметки, второй — для автоматизации этого процесса с помощью машинного обучения.

---

### Технологический стек №1: Подготовка данных и разметка в Label Studio

**Цель:** Преобразовать сырой видеофайл в набор структурированных, размеченных экспертом данных (`labeled.csv`). Этот стек ориентирован на обработку данных и создание удобного интерфейса для человека.

| Компонент | Тип | Роль в проекте |
| :--- | :--- | :--- |
| **Ubuntu 22.04 LTS** | Операционная система | Стабильная и надежная среда для развертывания всех инструментов. |
| **Python 3** | Язык программирования | Язык-оркестратор, который связывает все библиотеки в единый пайплайн. |
| **FFmpeg** | Системная утилита | **Высокоскоростная нарезка** видео на крупные части (10-минутные фрагменты) без перекодирования (`-c copy`). Основа производительности. |
| **MoviePy** | Python-библиотека | **Детальная работа с сегментами:** нарезка выбранного фрагмента на мелкие 5-секундные части и точное извлечение аудиодорожек из них. |
| **OpenCV** | Python-библиотека | **Анализ видеоряда:** извлечение кадров из сегментов и выбор **ключевого кадра** на основе анализа четкости (дисперсия Лапласа). |
| **Vosk** | Python-библиотека / Модель | **Распознавание речи (Speech-to-Text):** преобразование извлеченных аудиосегментов в текстовую транскрипцию (работает офлайн). |
| **Librosa** | Python-библиотека | **Анализ аудиосигнала:** извлечение числовых признаков из аудио, таких как средняя громкость (RMS), для дальнейшего анализа. |
| **vaderSentiment** | Python-библиотека | **Анализ текста (NLP):** вычисление сентимента (эмоциональной окраски) на основе транскрибированного текста. |
| **Pandas** | Python-библиотека | **Структурирование данных:** организация всех извлеченных признаков (аудио, текст) в таблицы и их сохранение в `.csv` файлы. |
| **Label Studio** | Платформа для разметки | **Центральный инструмент эксперта:** предоставляет веб-интерфейс для комплексной оценки данных (видео, аудио, текст) и присвоения им меток (паттернов). |

#### Workflow (Поток данных):
`Видеофайл` → **FFmpeg** (быстрая нарезка) → `Выбранный фрагмент` → **MoviePy** (нарезка на 5с сегменты) → **OpenCV** (извлечение кадра) + **Librosa** (анализ аудио) + **Vosk** (транскрибация) → **vaderSentiment** (анализ текста) → **Pandas** (сборка признаков) → `tasks.json` → **Label Studio** (разметка экспертом) → `labeled.csv`.

---

### Технологический стек №2: Автоматизация разметки с помощью нейросети

**Цель:** Создать и применить модель машинного обучения, которая автоматически размечает новые, невиданные ранее видеокадры, имитируя работу эксперта. Этот стек ориентирован на глубокое обучение (Deep Learning).

| Компонент | Тип | Роль в проекте |
| :--- | :--- | :--- |
| **Python 3 / Ubuntu** | Язык / ОС | Основа для работы фреймворков машинного обучения. |
| **TensorFlow (с Keras API)** | Фреймворк для ML | **"Движок" нейросети:** предоставляет все необходимые инструменты для построения, компиляции, обучения и применения модели компьютерного зрения. |
| **MobileNetV2** | Предобученная модель | **"Зрительная кора" нашей системы:** мы используем эту мощную, предобученную на миллионах изображений нейросеть как основу (**Transfer Learning**), чтобы не обучать нашу модель с нуля, а лишь дообучить ее на наших специфических задачах. |
| **Scikit-learn** | Библиотека для ML | **Подготовка данных для обучения:** используется для разделения данных на обучающую и валидационную выборки (`train_test_split`) и для преобразования текстовых меток в числовой формат, понятный нейросети (`MultiLabelBinarizer`). |
| **Pillow (PIL)** | Python-библиотека | **Работа с изображениями:** загрузка, изменение размера и преобразование ключевых кадров в формат, который "понимает" TensorFlow. |
| **NumPy** | Python-библиотека | **Числовые вычисления:** фундаментальная библиотека для работы с многомерными массивами (тензорами), на которой построены и TensorFlow, и Pandas. |
| **Pandas** | Python-библиотека | **Обработка данных:** используется на этапе обучения для чтения `labeled.csv` и на этапе предсказания для структурирования итогового отчета. |
| **OpenCV / MoviePy** | Python-библиотеки | **Подготовка данных для предсказания:** на этапе применения модели (inference) эти библиотеки используются для извлечения ключевых кадров из нового видеофайла. |

#### Workflow (Поток данных):
1.  **Обучение:** `labeled.csv` + `processed_data/` (кадры) → **Pandas/Scikit-learn** (загрузка и кодирование меток) → **TensorFlow/Keras** (создание модели на базе MobileNetV2 и обучение) → `image_pattern_model.h5` (сохраненная модель).
2.  **Предсказание:** `Новое видео` → **MoviePy/OpenCV** (извлечение кадров) → **TensorFlow** (загрузка модели и предсказание) → `ТОП-3 паттерна с вероятностями` → `metadata.jsonl` (финальный отчет).
