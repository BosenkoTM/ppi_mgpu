...
**Pedagogical Pattern Infrastructure (PPI)** — веб-платформа для обработки видео, автоматической транскрибации и подготовки датасетов для анализа педагогических паттернов.
...
- [Алгоритм загрузки данных и работы в Label Studio](#алгоритм-загрузки-данных-и-работы-в-label-studio)
...
Проект **PPI Video Processing Platform** предназначен для создания датасета педагогических паттернов на основе видео-материалов. Процесс работы разделен на три основных этапа:

### Этап 1: Загрузка данных в MinIO и разметка видео в Label Studio

**Цель:** Подготовка исходных данных и их разметка для дальнейшей обработки.

**Процесс:**
1. **Загрузка видео в MinIO**
   - Пользователь загружает видеофайлы через веб-интерфейс
   - Видео сохраняется в MinIO хранилище (бакет `raw-data`)
   - Извлекается аудиодорожка из видео с помощью FFmpeg
   - Аудио сохраняется в том же бакете
   - Опционально выполняется транскрибация аудио с помощью OpenAI Whisper

2. **Работа в Label Studio**
   - После загрузки видео автоматически генерируется presigned URL (действителен 7 дней)
   - Пользователь копирует URL и импортирует видео в Label Studio
   - В Label Studio выполняется разметка видео (аннотация педагогических паттернов)
   - Результаты разметки экспортируются в формате JSON

3. **Получение JSON разметки**
   - JSON файл с разметкой сохраняется в MinIO (бакет `teacher-patterns` или `student-patterns`)
   - JSON содержит аннотации, временные метки, метки классов и другую информацию о разметке

**Результат этапа:**
- Видеофайлы в MinIO (`raw-data`)
- Аудиофайлы в MinIO (`raw-data`)
- JSON файлы с разметкой в MinIO (`teacher-patterns` / `student-patterns`)

---

### Этап 2: Обработка видео на основе JSON + видео

**Цель:** Обработка размеченных видео для формирования структурированного датасета.

**Процесс:**
1. **Загрузка данных для обработки**
   - Загрузка видео из MinIO (`raw-data`)
   - Загрузка соответствующего JSON файла с разметкой (`teacher-patterns` / `student-patterns`)
   - Сопоставление видео и JSON по идентификаторам

2. **Обработка видео на основе разметки**
   - Парсинг JSON файла с аннотациями
   - Извлечение сегментов видео согласно временным меткам из JSON
   - Обработка каждого сегмента (извлечение кадров, аудио, транскрипция)
   - Формирование структурированных данных для каждого паттерна
   - Создание метаданных для датасета

3. **Подготовка датасета**
   - Организация данных в структурированном формате
   - Создание файлов конфигурации датасета
   - Валидация данных
   - Сохранение обработанных данных в MinIO (бакет `final-dataset`)

**Статус:** Модуль находится в разработке.

**Результат этапа:**
- Обработанные сегменты видео
- Структурированные данные для каждого паттерна
- Метаданные датасета
- Файлы конфигурации

---

### Этап 3: Выгрузка датасета в Hugging Face

**Цель:** Публикация готового датасета для использования в ML моделях.

**Процесс:**
1. **Описание датасета**
   - Создание описания датасета (README, метаданные)
   - Документирование структуры данных
   - Описание классов паттернов
   - Указание лицензии и авторов
   - Добавление примеров использования

2. **Подготовка к выгрузке**
   - Формирование финальной структуры датасета
   - Валидация всех данных
   - Создание файлов метаданных (dataset_info.json, README.md)
   - Подготовка примеров данных

3. **Выгрузка в Hugging Face**
   - Загрузка датасета в Hugging Face Hub
   - Публикация описания датасета
   - Настройка доступа (публичный/приватный)
   - Тестирование загрузки датасета

**Результат этапа:**
- Опубликованный датасет на Hugging Face
- Документация датасета
- Доступность для использования в ML моделях
